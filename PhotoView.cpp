...
void PhotoView::loadImageFile(QString& fileName, bool bRefreshDir)
{
   ...
   // qDebug()<<m_fileInfo.absolutePath()<<m_fileInfo.absoluteFilePath();//C:/Users/Tiger/Pictures/1.jpg
    if (m_fileInfo.isFile())
    {
        ...
        if (m_isPixmapLoaded)
        {
            std::string exifFileName = szFile.toStdString();
            bool ret = m_exifInfo.getExifInfo(exifFileName);
            //qDebug()<<"m_exifInfo.getExifInfo"<<ret;
            if (_exifPanel != nullptr)
            {
                std::string text = m_exifInfo.makeString();
                {
                    ...
                    QString location = "Location:" + m_fileInfo.path();
                    QLocale locale = QLocale::system();
                    QMimeDatabase mimedb;
                    QMimeType mime = mimedb.mimeTypeForFile(m_fileInfo.absoluteFilePath(), QMimeDatabase::MatchContent);
                    QString sType = "Type:" + mime.name();


                    //this is just math to figure the megapixels and then round it to the tenths place
                    const double megapixels = static_cast<double>(qRound(((static_cast<double>((m_readData.imageSize.width() * m_readData.imageSize.height()))))/1000000 * 10 + 0.5)) / 10 ;
                    QString sSize = QString("%1 (%2 bytes)").arg(formatBytes(m_fileInfo.size()), locale.toString(m_fileInfo.size()));
                    QString sModified = m_fileInfo.lastModified().toString(locale.dateTimeFormat());
                    QString sDemension = QString("%1 x %2 (%3 MP)").arg(QString::number(m_readData.imageSize.width()), QString::number(m_readData.imageSize.height()),
                                                                                       QString::number(megapixels));


                    std::stringstream ss;
                    ss << std::setw(30) << std::setfill(' ') << std::left <<"Name:" << ": "<< std::setfill(' ') << std::right << m_fileInfo.fileName().toStdString()<<'\n';
                    ss << std::setw(30) << std::setfill(' ') << std::left <<"Type:" << ": "<< std::setfill(' ') << std::right << mime.name().toStdString()<<'\n';
                    ss << std::setw(30) << std::setfill(' ') << std::left <<"Location:" << ": "<< std::setfill(' ') << std::right << m_fileInfo.path().toStdString()<<'\n';
                    ss << std::setw(30) << std::setfill(' ') << std::left <<"Size:" << ": "<< std::setfill(' ') << std::right << sSize.toStdString()<<'\n';
                    ss << std::setw(30) << std::setfill(' ') << std::left <<"Modified:" << ": "<< std::setfill(' ') << std::right << sModified.toStdString()<<'\n';
                    ss << std::setw(30) << std::setfill(' ') << std::left <<"Demension:" << ": "<< std::setfill(' ') << std::right << sDemension.toStdString()<<'\n';
                    ss<<"\r\n";

                    std::string info = ss.str();
                    info += text;

                    QString text2 = QString::fromStdString(info);
                    _exifPanel->setText(text2);
                }
            }
